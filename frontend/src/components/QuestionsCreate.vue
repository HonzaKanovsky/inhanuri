...
const submitForm = async () => {
  ...
  try {
    ...
    router.push("/questions"); // Redirect after success
  } catch (error) {
    console.error("Error creating questions post:", error);
...
<template>
  <section class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
    <h1 class="text-3xl font-bold mb-6 text-center">Ask question</h1>
    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="mb-4">
        ...
        <input v-model="password" type="password" class="w-full p-2 border rounded" required pattern="\d{4}" title="PIN must be a 4-digit number" />
        <p class="text-gray-500 text-sm mt-1">* This PIN can be used later to edit the post. It must be a 4-digit number.</p>
      </div>
